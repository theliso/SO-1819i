...
VOID RemoveThreadAlive() {
	for (LIST_ENTRY *list = AliveList.Flink; list != &AliveList; list = list->Flink) {
		if ((PUTHREAD)list == RunningThread) {
...
BOOL UtAlive(HANDLE thread) {
	for (LIST_ENTRY *list = AliveList.Flink; list != &AliveList; list = list->Flink) {
		if ((PUTHREAD)list == (PUTHREAD)thread) {
			return TRUE;
		}
	}
	return FALSE;
...
VOID UtSwitchTo(HANDLE threadToRun) {
	...
	if (thread->State == READY) {
		UtActivate((HANDLE)RunningThread);
		ContextSwitch(RunningThread, thread);
	}
}

// Remove from the blocked list and insert in the ready queue
BOOL UtJoinCancel(HANDLE thread) {
	PUTHREAD threadToCancel = (PUTHREAD)thread;
	if (!UtAlive(thread) || threadToCancel->BlockedCount == 0) {
		return FALSE;
	}
	threadToCancel->Cancelled = TRUE;
	threadToCancel->ThreadState = READY;
	RemoveEntryList(threadToCancel->Block);
	UtActivate(thread);
	return TRUE;
}
...
DWORD UtJoin(HANDLE thread) {
	if (thread == UtSelf() || ((PUTHREAD)thread)->Cancelled) {
		return -1;
	}
	if (!UtAlive(thread)) {
		return 0;
	}
	PUTHREAD JoinThread = (PUTHREAD)thread;
	RunningThread->ThreadState = BLOCKED;
	InsertTailList(&JoinThread->Block, &RunningThread->Block);
	UtDeactivate();
	ContextSwitch(RunningThread, JoinThread);
	return 1;
}
...
HANDLE UtCreate (UT_FUNCTION Function, UT_ARGUMENT Argument) {
	...
	InsertTailList(&AliveList, &Thread->Link);
